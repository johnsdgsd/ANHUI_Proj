...
class LocalWarehouseInitializer:
    ...
    def load_city_mapping(self, init_stock_df: pd.DataFrame):
        ...
        Args:
            init_stock_df: 初始库存数据DataFrame，包含 UNIT_CODE 和 UNIT_NAME 列
        """
        if 'UNIT_CODE' not in init_stock_df.columns:
            raise ValueError("init_stock_df 必须包含 UNIT_CODE 列")
        
        if 'UNIT_NAME' not in init_stock_df.columns:
            raise ValueError("init_stock_df 必须包含 UNIT_NAME 列")
        
        unique_cities = init_stock_df[['UNIT_CODE', 'UNIT_NAME']].drop_duplicates()
    ...
    def initialize_warehouses(self, init_stock_df: pd.DataFrame) -> list[LocalWarehouse]:
        ...
        Args:
            init_stock_df: 初始库存数据DataFrame，包含 UNIT_CODE 列
        ...
        city_codes = init_stock_df['UNIT_CODE'].dropna().unique()
